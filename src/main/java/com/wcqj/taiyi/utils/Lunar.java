...
//		 cal.setTime(sdf.parse("2016-05-28 16:00"));
...
//        System.out.println(ganziyear+" "+ganzimonth+" "+ganziday+" "+ganzitime);
     ...
     */
    public  int getJiJie(){
    		Date[] dateList = jieqilist(this.solarYear);//24个节气；
    	
    		for(int i=0;i<dateList.length;i++){
//    			System.out.println(DateUtil.getYYYYMMDDHHMISS(new Timestamp(dateList[i].getTime())));
    		}
    		int index = 0;
    		for(int i=0;i<dateList.length;i++){
    			if( this.solar.getTime().before( dateList[i]) ){
    				index = i - 1;
    				break;
    			}
    		}
